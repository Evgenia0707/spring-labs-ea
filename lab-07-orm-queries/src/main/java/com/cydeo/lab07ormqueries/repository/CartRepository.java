...
public interface CartRepository extends JpaRepository<Cart, Long> {
    ...
    List<Cart> fetchCartListByCustomer(Long id);
    ...
    @Query(value = "SELECT * FROM cart c JOIN customer cu ON c.customr_id = cu.id" +
            "WHERE c.cart_state = ?1 AND cu.id =?2 AND discount.id <> null" , nativeQuery = true)
//    "SELECT * FROM cart WHERE cart_state = ?1 AND customer_id = ?2 AND discount_id <> null"
