...
public class CartServiceImpl implements CartService {
    ...
    @Override
    public List<CartItem> retrieveCartDetail(UUID cartId) {//return all cart list
        // todo implement method using stream

//        return CART_LIST.stream()
//                .filter(cart -> cart.getId().equals(cartId.toString()))//toString- bec UUID
...
//        return CART_LIST.stream().filter(cart -> cart.getId().toString().equals(cartId.toString()))
//                .flatMap(cart -> cart.getCartItemList().stream()).collect(Collectors.toList());
...
//        }
        BigDecimal cart2TotalAmount = cartItemList.stream()
                .map(cartItem -> cartItem.getProduct().getPrice().multiply(BigDecimal.valueOf(cartItem.getQuantity())))
                .reduce(BigDecimal::add).orElseThrow();
